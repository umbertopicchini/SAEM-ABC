function [THETAmatrix_saem,Fisher_info,xhat_selected] = saem_pmcmc(model_param,parmask,parbase,yobs,saem_numit,warmup,fisherestim_iter,numparticles,N_threshold)
% SAEM-SMC, that is SAEM using the bootstrap filter.
% See % in Picchini & Samson 2017, Coupling stochastic EM and Approximate Bayesian Computation for parameter inference in state-space models, arXiv:1512.04831.
% 
...
for saem_iter = 2:saem_numit+1
   ...
   % obtain a state trajectory using the SMC sampler
   xhat_selected = smc_filter(model_param,yobs,numparticles,N_threshold);
