% runs 50 estimation procedures using SAEM-ABC for the example in section 4.2
...
% in particular, the settings below reproduce the SAEM-ABC(1) results in
% table 3 when using M=200, \bar{M}=10.
...
fisherestim_iter = 20; % ignore, this is not used in the paper.
abc_schedule = [1 80 150 200]; % interpret this together with abc_vector below: from SAEM iterations between 1 and 80 we use delta = 0.5,
                               % from iteration 80 to iteration 150 we use delta=0.2,
                               % from iteration 150 to iteration 200 we use delta=0.1,
                               % from iteration 200 to iteration saem_numit we use delta=0.03.
abc_vector =   [0.5 0.2 0.1 0.03];   % the DECREASING sequence of ABC thresholds delta
verbose = 1;  % can be 0/1. If 1 some output will be printed on screen at each iteration of SAEM
...
initstate = X0; 
   ...
   THETAmatrix_saem = saem_abcsmc(model_param,parmask,parbase,yobs,initstate,saem_numit,warmup,fisherestim_iter,numparticles,N_threshold,abc_schedule,abc_vector,verbose);
